...
export function SummaryScreen() {
  ...
  const signalsByLevel = {
    critical: sensorActiveSignals.filter(
      (s) => s.level === "critical",
    ),
    high: sensorActiveSignals.filter((s) => s.level === "high"),
    medium: sensorActiveSignals.filter(
      (s) => s.level === "medium",
    ),
  ...
  const getRecommendations = () => {
    ...
    if (
      signalsByLevel.critical.length > 0 ||
      signalsByLevel.high.length > 0
    ) {
      recommendations.push(
        "Consider consulting a healthcare professional for the high-severity areas.",
      );
    }
    if (
      sensorActiveSignals.some(
        (s) =>
          s.type.toLowerCase().includes("tension") ||
          s.type.toLowerCase().includes("muscle"),
      )
    ) {
      recommendations.push(
        "Regular stretching and posture checks may help reduce muscle tension.",
      );
    }
    if (
      sensorActiveSignals.some((s) =>
        s.type.toLowerCase().includes("headache"),
      )
    ) {
      recommendations.push(
        "Ensure adequate hydration and rest. Monitor for recurring patterns.",
      );
    }
    if (sensorActiveSignals.length > 3) {
      recommendations.push(
        "Multiple pain points detected. Consider a comprehensive wellness check.",
      );
    }
      ...
      <div className="px-6 space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center p-3 rounded-xl bg-stone-800/50">
              <div className="text-2xl font-bold text-red-400">
                {signalsByLevel.critical.length +
                  signalsByLevel.high.length}
              </div>
              <div className="text-xs text-stone-500">
                High Priority
              </div>
          ...
          <div className="mt-4 space-y-2">
            {Object.entries(signalsByLevel).map(
              ([level, signals]) =>
                signals.length > 0 && (
                  <div
                    key={level}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${SEVERITY_COLORS[level as keyof typeof SEVERITY_COLORS].bg}`}
                      />
            ...
            {sensorActiveSignals.map((signal) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2 h-8 rounded-full ${SEVERITY_COLORS[signal.level].bg}`}
                  />
                  <div>
                ...
                <span
                  className={`text-xs font-medium ${SEVERITY_COLORS[signal.level].text} capitalize`}
                >
                  {signal.level}
                </span>
              </div>
